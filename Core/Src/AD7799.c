...
{
	if ((AD7799_GetRegisterValue(ad7799, AD7799_REG_ID, 1) & 0x0F) != AD7799_ID) return HAL_ERROR;
 ...
 */
uint32_t AD7799_GetRegisterValue(AD7799 *ad7799, uint8_t regAddress, uint8_t size)
{
 ...
 * @param size
 */
void AD7799_SetRegisterValue(AD7799 *ad7799, uint8_t regAddress, uint32_t regValue, uint8_t size)
{
	uint8_t data[4] = { 0x00, 0x00, 0x00, 0x00 };
...
HAL_StatusTypeDef AD7799_SingleConversion(AD7799 *ad7799)
{
	...
	{
		if (HAL_GetTick() - startTime > (uint32_t) (1.5 * settle_time_ms[(uint8_t) ad7799->rate]))
		{
			return HAL_TIMEOUT;
		}
	}
